...
bool cmp(const project& a, const project& b) {
    int ca = 1.0 * max(0, a.score - max(0, t + a.len - a.dead)) / (a.len / 2) / a.req.size();
    int cb = 1.0 * max(0, b.score - max(0, t + b.len - b.dead)) / (b.len / 2) / b.req.size();
    return ca > cb;
}
