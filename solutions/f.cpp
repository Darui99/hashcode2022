...
bool cmp(const project& a, const project& b) {
    double ca = 1.0 * max(0, a.score - max(0, t + a.len - a.dead)) / (a.len / 5) / a.req.size();
    double cb = 1.0 * max(0, b.score - max(0, t + b.len - b.dead)) / (b.len / 5) / b.req.size();
    return ca > cb;
}
...
int main()
{
    in("f_find_great_mentors.in.txt");
    out("foutput.txt");
    srand(1373737);
    ...
    while (t <= maxdead) {
        ...
        if (t % 1000 == 0)        sort(projects.begin(), projects.end(), cmp);

        bool any = false;
        bool anyCost = false;

//        cerr << " r " << cmp(projects[0], projects[1]) << endl;
//        for (int i = 0; i < 10; ++i) {
//            cerr << " TTT " <<  projects[i].score << " " << projects[i].dead << " " << projects[i].len << " " << projects[i].req.size() << endl;
//        }
