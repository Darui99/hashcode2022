...
bool cmp(const project& a, const project& b) {
    int ca = max(0, a.score - max(0, t + a.len - a.dead)) * 10 / a.req.size();
    int cb = max(0, b.score - max(0, t + b.len - b.dead)) * 10 / b.req.size();
    return ca > cb;
}
